...
const R = {
  ...
  tagline: "30 años alimentando el Polígono do Tambre",
  description:
...
const SERVICES = [
  ...
  { icon: "🛍️", title: "Para Llevar", desc: "Pide tu menú para llevarte a la oficina o a casa. Sin esperas." },
];
      ...
      {[1, 2, 3, 4, 5].map((i) => {
        ...
        return (
          <svg key={i} viewBox="0 0 20 20" className="w-4 h-4">
            ...
            <path
              fill={filled ? "#F59E0B" : half ? `url(#${gradId})` : "#D1D5DB"}
    ...
    <>
      ...
      >
        ...
        >
          ...
          {menuOpen && (
            <div id="mobile-menu" style={{ background: "rgba(15,30,60,0.98)", borderTop: "1px solid rgba(255,255,255,0.08)" }}
              className="md:hidden px-5 pb-6 pt-3 flex flex-col gap-2">
              {NAV_LINKS.map(([id, label]) => (
                <button key={id} onClick={() => scrollTo(id)}
                  className="text-left text-white/85 font-medium py-3 text-base">{label}</button>
              ))}
              <Link to="/carta" onClick={() => setMenuOpen(false)}
                className="inline-block w-fit px-5 py-2.5 rounded-full font-semibold text-sm mt-2"
                style={{ background: "#D4A017", color: "#1B2F5A" }}>
                Ver Carta Digital
              </Link>
            </div>
          ...
          <div className="relative z-10 max-w-6xl mx-auto px-5 w-full">
            <div className="hero-rating flex items-center gap-3 mb-5">
              <div className="flex items-center gap-2 rounded-full px-4 py-1.5" style={{ background: "rgba(212,160,23,0.15)", border: "1px solid rgba(212,160,23,0.35)" }}>
                ...
                <span className="text-white/80 text-sm font-semibold">{R.reviewCount.toLocaleString("es-ES")} personas lo recomiendan</span>
              </div>
            </div>
            ...
            <div className="hero-ctas flex flex-wrap gap-3 mt-8">
              ...
              <a href={R.phoneTel}
                className="flex items-center gap-2 px-6 py-3 rounded-full font-semibold text-sm border text-white transition-all hover:bg-white/10"
              ...
              {SERVICES.map(({ icon, title, desc }, i) => (
                <Reveal key={title} delay={i * 80}>
                  <div className="rounded-2xl p-6 h-full transition-all duration-300 hover:-translate-y-2 hover:shadow-xl"
                    ...
                    <h3 style={{ fontFamily: "'Rufina', serif", fontSize: "1.2rem", fontWeight: 700, color: "#1B2F5A" }}>{title}</h3>
        ...
        {/* ── PRE-FOOTER CTA ── */}
        <section style={{ background: "#1B2F5A" }} className="py-14">
          <div className="max-w-2xl mx-auto px-5 text-center">
            <p style={{ color: "#D4A017", fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", fontSize: "0.75rem" }}>
              ¿Tienes hambre?
            </p>
            <h2 style={{ fontFamily: "'Rufina', serif", fontSize: "clamp(1.6rem,3.5vw,2.4rem)", fontWeight: 700, color: "#fff", marginTop: "0.4rem", lineHeight: 1.2 }}>
              Reserva tu mesa o pide para llevar
            </h2>
            <div className="mt-6 flex flex-wrap justify-center gap-3">
              <a href={R.phoneTel}
                className="flex items-center gap-2 px-7 py-3.5 rounded-full font-semibold text-sm transition-all hover:scale-105 hover:brightness-110"
                style={{ background: "#D4A017", color: "#1B2F5A", boxShadow: "0 4px 20px rgba(212,160,23,0.35)" }}>
                <Phone className="w-4 h-4" />
                {R.phone}
              </a>
              <Link to="/carta"
                className="flex items-center gap-2 px-7 py-3.5 rounded-full font-semibold text-sm border transition-all hover:bg-white/10"
